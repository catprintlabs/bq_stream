module BqStream
  module AutoCreate
    def needs_init?
      ...
      return false if BqStream.on_console?
  ...
  class Connection < ActiveRecord::Base
    class QueuedItem < ActiveRecord::Base
      ...
      belongs_to :bq_stream_connection,
                 class_name: 'BqStream::Connection',
                 foreign_key: 'connection_id'

      scope :for_session,
            ->(session) { joins(:bq_stream_connection).where('session = ?', session) }
    ...
    def self.build_tables
      ...
      QueuedItem.create_table(force: true) do |t|
        t.text    :data
    ...
    self.table_name = 'bq_stream_connections'

    has_many :messages,
             foreign_key: 'connection_id',
             class_name: 'BqStream::Connection::QueuedMessage',
             dependent: :destroy
    scope :expired,
    ...
    class << self
      ...
      def send_to_channel(channel, data)
        pending_for(channel).each do |connection|
          QueuedItem.create(data: data, synchromesh_connection: connection)
        end
      ...
      def read(session, root_path)
        ...
        QueuedItem.for_session(session).destroy_all.pluck(:data)
      end
      ...
      def root_path=(path)
        QueuedItem.root_path = path if path
      end

      def root_path
        QueuedItem.root_path
      rescue
        nil
      end
