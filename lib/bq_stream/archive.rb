module BqStream
  module Archive
    ...
    def id_streamline_archive(back_date, override_dataset = nil, verify_oldest = false)
...
# end
    ...
    def old_id_streamline_archive(back_date, dataset = "#{ENV['DATASET']}_#{Rails.env}", verify_oldest = false)
      ...
      if verify_oldest
        ...
        update_revision.update(attr: current_deploy, archived: true)
        BqStream.log(:info, "#{Time.now}: ***** End Verifying Oldest Records *****")
      end
