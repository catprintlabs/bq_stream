module BqStream
  module Archive
    def id_streamline_archive(back_date, dataset = BqStream.dataset)
      ...
      # Initialize an empty buffer for records to go to BigQuery
      buffer = []
      ...
      until BqStream::OldestRecord.where(archived: false).empty? && buffer.empty?
        BqStream::OldestRecord.table_names.each do |table|
          ...
          if record_id_query['rows']
            ...
            earliest_record_id = table.constantize.find_by_sql("SELECT * FROM #{table.underscore.pluralize} ORDER BY id DESC LIMIT 1").first.try(:id)
          end
          ...
          back_date_id = table.constantize.find_by_sql("SELECT * FROM #{table.underscore.pluralize} WHERE (created_at >= '#{back_date}') ORDER BY id ASC LIMIT 1").first.try(:id)
          ...
          until BqStream::OldestRecord.where(table_name: table, archived: false).empty?
            ...
            next_batch = table.constantize.find_by_sql("SELECT * FROM #{table.underscore.pluralize} WHERE (id >= #{back_date_id} AND id <= #{earliest_record_id}) ORDER BY id DESC LIMIT #{10_000 / (oldest_attr_recs.count.zero? ? 1 : oldest_attr_recs.count)}")
